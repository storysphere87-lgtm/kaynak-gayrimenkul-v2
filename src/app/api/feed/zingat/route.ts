...
export async function GET() {
  try {
    ...
    // Zingat da Trovit standardını desteklemektedir, bu nedenle aynı şemayı
    // bağımsız bir endpoint üzerinden sunuyoruz (İzleme ve analiz kolaylığı için)
    const listingsXml = properties.filter(p => p.status === 'aktif').map(p => {
      const islemSlug = p.type?.toLowerCase() === 'satılık' ? 'satilik' : 'kiralik';
      const propertyUrl = `${baseUrl}/tr/portfoy/${p.district_id}/${islemSlug}/${p.id}?utm_source=zingat`;
...
${listingsXml}
    ...
    console.error('Zingat Feed Error:', error);
