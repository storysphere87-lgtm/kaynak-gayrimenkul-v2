...
export async function POST(request: Request) {
  ...
  const rateResult = checkRateLimit(ip, 'evaluate', RATE_LIMITS.evaluate);
  ...
  try {
    ...
    // 1. Veritabanından gerçek m² fiyat trendlerini çek
    const trends = await getMarketTrends();
    const districtTrend = trends?.find(
      (t: any) => t.district.toLowerCase() === districtName?.toLowerCase()
    ) || {
      avg_sqm_price:         32000,
    ...
    // 2. AI ile profesyonel pazar yorumu
...
Aşağıdaki verileri kullanarak bu gayrimenkulün piyasa değeri için profesyonel, ikna edici 3-4 cümlelik bir değerleme yorumu yaz.
...
Temiz Türkçe metin döndür. Markdown veya JSON kullanma.
`.trim();

    const result = await callAI(prompt);
    if (result) aiEvaluation = result.trim();

    // 3. Değerleme raporu (HTML/Word formatında)
    const htmlContent = `
...
<body>
  ...
  <table class="stats-table">
    <tr><td class="stats-header">Taşınmaz Bölgesi</td><td><strong>Ankara / ${districtName || 'Etimesgut'}</strong></td></tr>
    <tr><td class="stats-header">Brüt Alan / Metrekare</td><td><strong>${targetSqm} m²</strong></td></tr>
    <tr><td class="stats-header">Oda / Bölüm Sayısı</td><td><strong>${rooms || 'Belirtilmedi'}</strong></td></tr>
    <tr><td class="stats-header">Mülk Kategorisi</td><td><strong>${propertyType || 'Konut'}</strong></td></tr>
  </table>

  <h2>2. HİPER-LOKAL PİYASA VERİLERİ</h2>
  <table class="stats-table">
    <tr><td class="stats-header">Bölgesel Ortalama m² Birim Değeri</td><td>₺${districtTrend.avg_sqm_price?.toLocaleString('tr-TR')} / m²</td></tr>
    <tr><td class="stats-header">Fiyat Değişim Trendi</td><td>%${districtTrend.price_change_percentage > 0 ? '+' : ''}${districtTrend.price_change_percentage} (${districtTrend.trend_direction?.toUpperCase()})</td></tr>
  </table>
  ...
  <div class="analysis-box">"${aiEvaluation}"</div>

  <p style="font-size: 10pt; text-align: justify; color: #333; margin-top: 30px;">
    * Bu rapor, Kaynak Gayrimenkul veritabanındaki aktif piyasa verilerini ve lisanslı gayrimenkul analitik algoritmalarını harmanlayarak otonom olarak oluşturulmuştur.
    Resmi ekspertiz raporu niteliğinde olmayıp, piyasa konumlandırması için güvenilir referans teşkil eder.
  </p>

  <div class="footer-note">Kaynak Gayrimenkul A.Ş. Değerleme Departmanı - Tüm Hakları Saklıdır.</div>
</body>
</html>
    `.trim();
